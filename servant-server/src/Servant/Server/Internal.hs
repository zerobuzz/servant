      ...
      (first : _)
        -> do let querytext = parseMatrixText . encodeUtf8 $ T.tail first
                  param = case lookup paramname querytext of
                    ...
                    Just (Just v) -> parseQueryParamMaybe v -- if present, we try to convert to
                                          -- the right type
              route (Proxy :: Proxy sublayout) (passToServer subserver param)
      _   -> route (Proxy :: Proxy sublayout) (passToServer subserver Nothing)
    ...
    case parsePathInfo request of
      (first : _)
        -> do let matrixtext = parseMatrixText . encodeUtf8 $ T.tail first
                  ...
                  values = mapMaybe (convert . snd) parameters
              route (Proxy :: Proxy sublayout) (passToServer subserver values)
      _ -> route (Proxy :: Proxy sublayout) (passToServer subserver [])
    where paramname = cs $ symbolVal (Proxy :: Proxy sym)
          looksLikeParam (name, _) = name == paramname || name == (paramname <> "[]")
          convert Nothing = Nothing
          convert (Just v) = parseQueryParamMaybe v
      ...
      (first : _)
        -> do let matrixtext = parseMatrixText . encodeUtf8 $ T.tail first
                  param = case lookup paramname matrixtext of
                    ...
                    Nothing       -> False -- param not in the query string
              route (Proxy :: Proxy sublayout) (passToServer subserver param)
      _ -> route (Proxy :: Proxy sublayout) (passToServer subserver False)
    where paramname = cs $ symbolVal (Proxy :: Proxy sym)
          examine v | v == "true" || v == "1" || v == "" = True
                    | otherwise = False
